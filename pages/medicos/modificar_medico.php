...
require '../sesion/abre_sesion.php';
  ...
  $id = $_GET['id'];
  $query = "SELECT * FROM usuarios INNER JOIN medico ON usuarios.id_usuario = medico.usuario_id WHERE usuario_id = {$id}";
      ...
      if(!is_numeric($cp))
        ...
        if(empty($email)) $email = 'null'; else $email = "'".$email."'";
          ...
          $modif = crear_registro($conexion,$query);
           ...
           $query = "UPDATE `medico` SET `no_cedula`='{$cedula}',`id_especialidad`='{$especialidad}'  WHERE usuario_id={$id};";
              ...
              <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?id={$id}" ?>" role="form">
                ...
                <div class="box-body">
                  ...
                  <div class="box-header">
                      ...
                      //    $r = select($conexion,'tipo_usuario');
                          foreach ($r as $esp) {
                            ...
                            if(!empty($errores) && $especialidad == $esp['id_especialidad']) echo 'selected';
                              if ($ruser['id_especialidad']==$esp['id_especialidad']) 
                                echo 'selected';
                    ...
                    <input type="text" class="form-control" name="cedula" placeholder="Ingrese la cedula del medico" maxlength="8"
                           value="<?php if(!empty($errores)){ echo $cedula;} else {echo $ruser['no_cedula'];} ?>" required>
                  </div>
